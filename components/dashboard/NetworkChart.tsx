...
import type { HistoryRange, ServerHistory } from "@/lib/types";
...
/** 网络吞吐趋势:每台服务器单独展示(下行实线 / 上行虚线),一个图表 */
export function NetworkChart({
  ...
  const seriesDefs: SeriesDef[] = servers.flatMap((s) => [
    { key: `${s.name} 下行`, name: `${s.name} 下行`, color: s.color, dashed: false },
    { key: `${s.name} 上行`, name: `${s.name} 上行`, color: s.color, dashed: true },
  ]);

  /** 按采样桶对齐时间戳,合并所有服务器的 下行/上行 */
  const rows = useMemo(() => {
    ...
    for (const h of available) {
      ...
      for (const p of h.points) {
        ...
        row[`${name} 下行`] = Number(p.netInBps);
        row[`${name} 上行`] = Number(p.netOutBps);
        byTs.set(ts, row);
      }
    }
              ...
              <AreaChart data={rows} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                {seriesDefs.map((s) => (
                  <Area
                    ...
                    fill={s.color}
                    fillOpacity={s.dashed ? 0.05 : 0.12}
